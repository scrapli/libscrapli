...
/// The content and position (in the processed/cleaned haystack) of things we stripped from
/// haystack.
...
/// The result of a strip iterator step -- either a journal entry (a run of deleted
/// bytes), or done w/ the final compacted length of the haystack.
pub const AsciiAnsiControlStripIteratorNext = union(enum) {
    item: AsciiAnsiControlStripJournalEntry,
    done: usize,
};

/// Iterates the ascii/ansi strip state machine over a haystack, compacting kept bytes
/// down in place and yielding a journal entry for each contiguous run of deleted bytes.
/// The "journal" allows us to reconstruct the full "raw" haystack later if we keep it/and want
/// to.
pub const AsciiAnsiControlStripIterator = struct {
    ...
    /// Returns the next journal entry, or done (w/ the compacted length) when the
...
/// Strips ascii and ansi chars in place in the haystack. Uses the iterator just ignoring the
/// journal since we dont need to "reconstruct".
pub fn stripAsciiAndAnsiControlCharsInPlace(
    haystack: []u8,
    start_idx: usize,
) usize {
    var iter = AsciiAnsiControlStripIterator{
        .haystack = haystack[start_idx..],
    };
